...
MIN_CASH_RESERVE_PCT = 0.8265625      # exp189: calendar features; test top20 profile robustness
...
def train_and_eval(seed: int = 0) -> tuple:
    ...
    # exp189: canonical top20 calendar-feature strategy. exp187/188 profiles
    # showed top20 positive across all seeds with stronger median than top10.
    ...
    try:
        ...
        # local optimum.
        topn_broker = simulate_passive_topn(model, eval_feat, device, top_n=20, name="top20",
        ...
        if topn_broker.equity_curve and len(topn_broker.equity_curve) > 5:
            ...
            print(f"[experiment] canonical = top20_picker (final equity ${topn_broker.equity_curve[-1][1]:,.2f})", flush=True)
    except Exception as e:
