...
MIN_CASH_RESERVE_PCT = 0.8265625      # exp170: best top2 reserve; exclude broad ETF candidates
    ...
    *,
    ...
    for ts in sorted_ts:
        ...
        for sym, i_now in events_here:
            last_idx_by_sym[sym] = i_now
        if not picked:
            ready = [(sym, i_now) for sym, i_now in events_here if i_now >= C - 1 and sym not in {"SPY", "QQQ", "IWM"}]
...
def train_and_eval(seed: int = 0) -> tuple:
    ...
    # exp170: canonical top2 quarter-readiness with 82.65625% reserve, excluding broad ETFs as candidates.
