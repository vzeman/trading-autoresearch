...
MIN_CASH_RESERVE_PCT = 0.2875         # exp114: keep best reserve while allowing earlier top4 deployment
    ...
    *,
    ...
    for ts in sorted_ts:
        ...
        if not picked:
            ...
            if len(ready) >= max(top_n, len(features) // 3):
...
def train_and_eval(seed: int = 0) -> tuple:
    ...
    # exp114: canonical top4 can deploy once one-third of symbols are ready.
