...
def write_iteration_md(
    ...
    md.append("")
    md.append("| metric | meaning |")
    md.append("|---|---|")
    md.append("| Sharpe (median) | Median annualized risk-adjusted return across seeds. Higher is better; it rewards return per unit of volatility, not just raw profit. |")
    md.append("| Sharpe CI low (5%) | Conservative lower-confidence estimate from bootstrap/per-seed variation. This is the main keep/discard gate because it penalizes lucky or unstable runs. |")
    md.append("| Max drawdown (DD) | Worst peak-to-trough portfolio equity decline during the eval window. Less negative is safer; large negative DD means the strategy had a deep loss period. |")
    md.append("| Net PnL | Final profit/loss after fixed fees and modeled slippage/liquidity costs. Positive PnL alone is not enough; it must be robust and beat SPY. |")
    md.append("| % time above SPY | Fraction of eval timestamps where strategy equity was above buy-and-hold SPY/SP500 benchmark. Higher means the strategy stayed ahead for longer. |")
    md.append("| Trades / Fees | Number of executed trades and fixed $1 fees paid. More trades must overcome higher transaction costs. |")
    md.append("")
