...
**🟢 KEEP** · exp71: top5 ranking horizons (4,8,10)→(3,4) — shorter, less noisy

_2026-05-02 21:38 UTC · 407s wall_
...
**Decision reason:** ci_low=-1.0120 > prior best -1.1713

## Per-seed details

```
[evaluator] seed 0: sharpe=+1.549  dd=-8.86%  pnl=$+4,797.33  trades=3
[evaluator] seed 1: sharpe=+2.260  dd=-6.03%  pnl=$+6,725.17  trades=3
[evaluator] seed 2: sharpe=-0.432  dd=-3.01%  pnl=$-380.21  trades=1
```

## Equity curve (full eval window, ~73 days)

![weighted equity](../docs/weighted_beaa5e6.png)

## Equity curve (first month)

![weighted 1m](../docs/weighted_1m_beaa5e6.png)
...
**Median holdout sharpe: +0.322** (vs in-symbol +1.549)
...
```diff
beaa5e6 exp71: top5 picker ranking horizons (4,8,10) → (3,4)

exp70 KEPT with top5_picker default ranking_horizons=(4, 8, 10) — sums
predictions at 1-day + 5-day + 30-day horizons to rank symbols. The
30-day horizon is the noisiest (compounded uncertainty across 30 days).

This iter tries (3, 4) = 4h + 1d horizons only. Hypothesis: shorter
horizons compound less noise → more confident ranking → tighter top5
picks → higher sharpe.
