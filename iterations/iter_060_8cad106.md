...
_2026-05-02 13:46 UTC · 2842s wall_
...
![weighted equity](../docs/weighted_8cad106.png)

## Equity curve (first month)

![weighted 1m](../docs/weighted_1m_8cad106.png)

## Trader profile comparison

Same trained model evaluated under 5 different time-horizon strategies (and SPY benchmark).

| profile | sharpe | PnL ($) | PnL % | trades | DD % | horizon |
|---|---:|---:|---:|---:|---:|---:|
| **intraday** | -1.398 | $-218.82 | -0.44% | 12 | -0.46% | 2h |
| **intramonth** | -0.419 | $-1.47 | -0.00% | 2 | -0.01% | 30d |
| **intraweek** | -92.709 | $-44,215.10 | -88.43% | 26569 | -88.43% | 5d |
| **longterm** | -0.419 | $-1.47 | -0.00% | 2 | -0.01% | 30d |
| **spy_buyhold** | +1.007 | $+1,804.56 | +3.63% | 1 | -8.76% | - |

**Best active strategy by Sharpe: `intramonth`**
...
```diff
8cad106 exp60: multi-trader-profile comparison + restore N_SEEDS_OVERRIDE

User: 'compare intraday trader with intra week and intra month trader
vs long term investor and comparing results from all of them to SP500 results'.

Adds 5 trader profile simulators all using the SAME trained model:
1. intraday   — 2h horizon, exit by 1 trading day (390 bars)
2. intraweek  — 5d horizon, max 5-day hold
3. intramonth — 30d horizon, max 30-day hold
4. longterm   — 30d horizon, no time exit (until conviction breaks)
5. spy_buyhold — passive baseline

Each profile picks a horizon from the model's 11-horizon mh_head and
applies a profile-specific max-hold rule. All use realistic friction
(volume slippage + liquidity gate from exp57).

Per-seed results dumped to checkpoints/last_seed{seed}_profiles.json.
Driver renders a comparison table + best-active-strategy callout in iter md.

Also restores N_SEEDS_OVERRIDE in driver (was wiped by an earlier discard
reset). Now the driver patches BOTH prepare.N_SEEDS and evaluator.N_SEEDS
since evaluator does `from prepare import N_SEEDS` (binds value).

Re-applies exp59's model/horizon config (n_layers=4, 11 horizons) so
checkpoints from a fresh retrain will be compatible across iterations.


 autoresearch_driver.py |  66 +++++++++++++--
 experiment.py          | 224 +++++++++++++++++++++++++++++++++++++++++++++++--
