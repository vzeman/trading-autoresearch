...
_2026-05-02 14:40 UTC · 2806s wall_
...
![weighted equity](../docs/weighted_7db4e02.png)

## Equity curve (first month)

![weighted 1m](../docs/weighted_1m_7db4e02.png)
...
```diff
7db4e02 exp61: bigger model + 11 horizons + 4 trading profiles + 3 passive top-N pickers + SPY

Re-applies infrastructure that kept getting wiped by discard resets, AND
adds new passive top-N pickers based on exp60's finding that all active
strategies LOSE to SPY buy-and-hold.

Trader profiles (active rotation):
  intraday   — 2h horizon, exit by 1 day (390 bars)
  intraweek  — 5d horizon, max 5-day hold
  intramonth — 30d horizon, max 30-day hold
  longterm   — 30d horizon, no time exit

Passive top-N pickers (NEW exp61):
  top5_picker  — pick top 5 stocks by avg(1d,5d,30d) Sharpe at start, hold to end
  top10_picker — pick top 10
  top20_picker — pick top 20

Plus SPY buy-and-hold benchmark.

Hypothesis: if model has any STOCK-PICKING alpha (vs timing alpha which
exp60 ruled out), the top-N pickers should beat SPY without rotation cost.

Model: d_model=128, n_layers=4 (deeper for the 11-horizon multi-task head).
Cached pretrain unavailable (different mh_head dim than last KEEP), so this
is full retrain. ~50min on MPS+1seed.


 experiment.py | 287 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++-
 1 file changed, 283 insertions(+), 4 deletions(-)
```
