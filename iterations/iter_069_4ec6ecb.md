...
**🟢 KEEP** · exp69: canonical = top5_picker (exp68 proved it beats SPY across seeds)

_2026-05-02 21:20 UTC · 354s wall_
...
**Decision reason:** ci_low=-1.1710 > prior best -1.3508
...
![weighted equity](../docs/weighted_4ec6ecb.png)

## Equity curve (first month)

![weighted 1m](../docs/weighted_1m_4ec6ecb.png)
...
```diff
4ec6ecb exp69: switch canonical top20 → top5 picker (exp68 proved top5 is best)

exp68 multi-seed proved that with fresh ranking-loss-trained weights, the
top5_picker beats SPY across all 3 seeds:
  top5_picker:  sharpe +1.295  pnl +$2,944 (+5.89%)  ← BEST, beats SPY
  top20_picker: sharpe +1.037  pnl +$2,275 (+4.55%)  ← still beats SPY
  spy_buyhold:  sharpe +1.007  pnl +$1,805 (+3.63%)  ← benchmark

Concentration matters: 5 high-conviction picks dominated 20 diluted picks.
The model's signal is strongest at the very top of the ranking, then the
edge degrades. Switching canonical to top5 lets the driver judge what's
actually working.

Same fresh weights from exp68 — using cached pretrain. Only the broker
selection changes. Should produce sharpe ~+1.295 / pnl ~$2,944 → first
genuine multi-seed beat-SPY KEEP.
